...
arma::Mat<T> arr_to_mat(const py::array_t<T>& src) {
    ...
    T* data = dtls::validate_from_array_mat<T>(info);
    // copy and ensure fortran order
    data = dtls::steal_copy_array<T>(src.ptr());
    return dtls::arr_to_mat(info, data, true, false);
} /* arr_to_mat */
...
arma::Mat<T> arr_to_mat(py::array_t<T>&& src) {
    ...
    data = dtls::steal_andor_copy(src.ptr(), data);
    return dtls::arr_to_mat(info, data, true, false);
} /* arr_to_mat */
...
arma::Mat<T> arr_to_mat(py::array_t<T>& src, bool copy = false, bool strict = false) {
    ...
    if (!well_behaved(obj) || copy) {
        // copy and ensure fortran order
        data = dtls::steal_copy_array<T>(obj);
        return dtls::arr_to_mat(info, data, true, strict);
    }
    return dtls::arr_to_mat(info, data, false, strict);
} /* arr_to_mat */
...
arma::Col<T> arr_to_col(const py::array_t<T>& src) {
    py::buffer_info info = src.request();
    T* data = dtls::validate_from_array_col<T>(info);
    // copy and ensure fortran order
    data = dtls::steal_copy_array<T>(src.ptr());
    return dtls::arr_to_col(info, data, true, false);
} /* arr_to_col */
...
arma::Col<T> arr_to_col(py::array_t<T>& src, bool copy = false, bool strict = false) {
    ...
    if (!well_behaved(obj) || copy) {
        // copy and ensure fortran order
        data = dtls::steal_copy_array<T>(obj);
        return dtls::arr_to_col(info, data, true, strict);
    }
    return dtls::arr_to_col(info, data, false, strict);
} /* arr_to_col */
...
arma::Row<T> arr_to_row(const py::array_t<T>& src) {
    py::buffer_info info = src.request();
    T* data = dtls::validate_from_array_row<T>(info);
    // copy and ensure fortran order
    data = dtls::steal_copy_array<T>(src.ptr());
    return dtls::arr_to_row(info, data, true, false);
} /* arr_to_row */
...
arma::Row<T> arr_to_row(py::array_t<T>& src, bool copy = false, bool strict = false) {
    ...
    if (!well_behaved(obj) || copy) {
        // copy and ensure fortran order
        data = dtls::steal_copy_array<T>(obj);
        return dtls::arr_to_row(info, data, true, strict);
    }
    return dtls::arr_to_row(info, data, false, strict);
} /* arr_to_row */
...
arma::Cube<T> arr_to_cube(const py::array_t<T>& src) {
    ...
    py::buffer_info info = src.request();
    T* data = dtls::validate_from_array_cube<T>(info);
    // copy and ensure fortran order
    data = dtls::steal_copy_array<T>(src.ptr());
    return dtls::arr_to_cube(info, data, true, false);
} /* arr_to_cube */
...
arma::Cube<T> arr_to_cube(py::array_t<T>& src, bool copy = false, bool strict = false) {
    ...
    if (!well_behaved(obj) || copy) {
        // copy and ensure fortran order
        data = dtls::steal_copy_array<T>(obj);
        return dtls::arr_to_cube(info, data, true, strict);
    }
    return dtls::arr_to_cube(info, data, false, strict);
} /* arr_to_cube */
...
inline py::array_t<T> row_to_arr(const arma::Row<T>& src) {
    ...
    return dtls::construct_array<T>(data);
} /* row_to_arr */
...
inline py::array_t<T> mat_to_arr(const arma::Mat<T>& src) {
    return dtls::construct_array<T>(new arma::Mat<T>(src));
} /* mat_to_arr */

template <typename T>
inline py::array_t<T> mat_to_arr(arma::Mat<T>&& src) {
    return dtls::construct_array<T>(new arma::Mat<T>(std::move(src)));
} /* mat_to_arr */
